...
then
  # Setting-up /u01 disk and disk /02
  sh /vagrant/scripts/01_setup_u01.sh $BOX_DISK_NUM virtualbox
  BOX_DISK_NUM=$((BOX_DISK_NUM + 1))
  sh /vagrant/scripts/01_setup_u02.sh $BOX_DISK_NUM virtualbox
  # Install OS Pachages
  sh /vagrant/scripts/02_install_os_packages.sh
  ...
  then
  echo "-----------------------------------------------------------------"  
  echo -e "${INFO}`date +%F' '%T`: Create The Database "      
  echo "-----------------------------------------------------------------" 
  # database setup 
  sh /vagrant/scripts/13_Setup_db.sh
  fi 
fi
