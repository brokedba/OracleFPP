...
# puts ""
# puts "│▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒│"
# puts " Oracle FPP (Fleet Patching and Provisioning) Vagrant box for KVM/libVirt or VirtualBox  "                                    
# puts "                             Let the Patching party begin                                "
# puts "│▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒│"
# puts ""
# puts ""
...
var_vm2_deploy_db   = params['host2']['deploy_db']
var_db_base         = params['host2']['oracle_base']         
var_db_home         = params['host2']['db_home']  
var_db_sid          = params['host2']['db_sid']          
var_db_PDB          = params['host2']['pdb']   
var_db_charset      = params['host2']['charset'] 
var_oracle_edition  = params['host2']['edition']      
var_listener_port   = params['host2']['listener_port']   
#
...
if var_provider_name == 'virtualbox'
  box_disks_num = 2
  var_box = 'ol74'
  var_url = 'https://yum.oracle.com/boxes/oraclelinux/ol74/ol74.box'
  ...
  #var_url = 'http://ca-at-170.ie.oracle.com:8080/boxes/oraclelinux-7.json'
...
if var_provider_name == 'virtualbox'
  ...
  var_vm2_u02_disk       = params['host2']['u02_disk']
...
Vagrant.configure(VAGRANTFILE_API_VERSION) do |config|
  ...
  if var_provider_name == 'virtualbox'
    ...
    config.vm.define :host1 do |host1|
      host1.vm.hostname = var_vm1_name
      if (var_network == 'hostonly')
        host1.vm.network :private_network, ip: var_vm1_public_ip, virtualbox__intnet: "public", virtualbox__hostonly: "vagrantnet0"
      end
      if (var_network == 'public')
    ...
    if var_vm2_deploy == 'true'
      config.vm.define :host2 do |host2|
        host2.vm.hostname = var_vm2_name
        if (var_network == 'hostonly')
          host2.vm.network :private_network, ip: var_vm2_public_ip, virtualbox__intnet: "public", virtualbox__hostonly: "vagrantnet0"
        end
        if (var_network == 'public')
        ...
        # Make u02 vdisk
          unless File.exist?(var_vm2_u02_disk)
            vb.customize ['createhd', '--filename', var_vm2_u02_disk, '--size', 100 * 1034]
          end
          vb.customize ['storageattach', :id, '--storagectl', 'SATA Controller', '--port', disk_num, '--device', 0, '--type', 'hdd', '--medium', var_vm2_u02_disk, '--nonrotational', var_non_rotational]
        end
      end
    end
  end
  ...
  config.vm.provision "shell", path: "scripts/setup.sh", env: {
      ...
      # host2 db     
       "DEPLOY_DB"           => var_vm2_deploy_db,   
       "DB_BASE"             => var_db_base,         
       "DB_HOME"             => var_db_home,         
       "ORACLE_SID"          => var_db_sid,          
       "ORACLE_PDB"          => var_db_PDB,          
       "ORACLE_CHARACTERSET" => var_db_charset,      
       "ORACLE_EDITION"      => var_oracle_edition,  
       "LISTENER_PORT"       => var_listener_port    
  }
